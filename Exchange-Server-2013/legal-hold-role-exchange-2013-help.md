---
title: 'Rôle de conservation légale: Exchange 2013 Help'
TOCTitle: Rôle de conservation légale
ms:assetid: c98ce8ca-3477-479a-ad23-a8e6459bc4d0
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Dd876939(v=EXCHG.150)
ms:contentKeyID: 50479207
ms.date: 05/23/2018
...
Le rôle de gestion `Legal Hold` permet aux administrateurs de déterminer si les données d'une boîte aux lettres doivent être conservées en cas de litige dans une organisation.
...
En plus des étendues de destinataire, l’étendue de la cmdlet **Enable-Mailbox** (incluse avec ce rôle) est également définie au moyen d’étendues de configuration de base de données. Les étendues de configuration de base de données contrôlent les bases de données dans lesquelles la cmdlet peut créer de nouvelles boîtes aux lettres. La base de données dans laquelle vous créez une boîte aux lettres doit figurer dans l’étendue de base de données. Cette condition s’applique lorsque vous spécifiez une base de données à l’aide du paramètre *Database* dans la cmdlet **Enable-Mailbox** ou si vous autorisez la fonctionnalité de distribution automatique de boîtes aux lettres à choisir la base de données pour vous. Pour plus d’informations, voir [Présentation des portées du rôle de gestion](understanding-management-role-scopes-exchange-2013-help.md).
