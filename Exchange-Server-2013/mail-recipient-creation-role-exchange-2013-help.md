---
title: 'Rôle Création de destinataires de message: Exchange 2013 Help'
TOCTitle: Rôle Création de destinataires de message
ms:assetid: 81081845-9ebf-4888-8f9f-fe7cf2704486
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Dd876915(v=EXCHG.150)
ms:contentKeyID: 50478579
ms.date: 04/24/2018
...
Ce rôle ne vous permet pas d’attribuer une extension messagerie aux dossiers publics. Pour doter les dossiers publics d’une extension messagerie, vous devez utiliser le rôle `Mail Enabled Public Folders`. Pour plus d’informations, voir [Rôle des dossiers publics à extension messagerie](mail-enabled-public-folders-role-exchange-2013-help.md).

Si votre organisation utilise un modèle d’autorisations fractionnées RBAC (Contrôle d’accès basé sur un rôle) où la création des destinataires est opérée par un groupe différent de celui qui opère la gestion des destinataires, attribuez le rôle `Mail Recipient Creation` au groupe de rôles de gestion qui effectue la création des destinataires, et le rôle `Mail Recipients` au groupe de rôles qui effectue la gestion des destinataires.

Si votre organisation a activé des autorisations fractionnées Active Directory, tous les rôles de gestion n’ayant pas valeur de délégation et attribués à ce rôle de gestion ont été supprimés. Lorsque des autorisations fractionnées Active Directory sont activées, seuls des administrateurs Active Directory exploitant les outils de gestion Active Directory peuvent créer de nouveaux principaux de sécurité, tels que des utilisateurs et des groupes de sécurité.

Pour plus d’informations sur le Contrôle d’accès basé sur un rôle (RBAC) et les autorisations fractionnées Active Directory, consultez la rubrique [Présentation des autorisations fractionnées](understanding-split-permissions-exchange-2013-help.md).
