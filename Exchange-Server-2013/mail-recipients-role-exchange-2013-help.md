---
title: 'Rôle des destinataires de messagerie: Exchange 2013 Help'
TOCTitle: Rôle des destinataires de messagerie
ms:assetid: 7cf1f1c7-4f32-4446-ac04-99a7f9df2059
ms:mtpsurl: https://technet.microsoft.com/fr-fr/library/Dd876911(v=EXCHG.150)
ms:contentKeyID: 50478543
ms.date: 04/24/2018
...
Le rôle de gestion `Mail Recipients` permet aux administrateurs de gérer les boîtes aux lettres, les utilisateurs de messagerie et les contacts existants au sein d’une organisation. Ce rôle ne peut pas créer ces destinataires. Utilisez le rôle `Mail Recipient Creation` pour les créer.

Ce type de rôle ne vous permet pas de gérer des dossiers publics à extension messagerie ou des groupes de distribution. Utilisez les rôles suivants pour gérer ces objets :
...
Si votre organisation dispose d’un modèle d’autorisations fractionnées qui permet de répartir la création et la gestion entre différents groupes, affectez le rôle `Mail Recipient Creation` au groupe chargé de la création de destinataires, et le rôle `Mail Recipients` au groupe qui prend en charge la gestion des destinataires. Pour plus d’informations, consultez les rubriques suivantes :
...
Outre les portées de destinataires, les cmdlets **Connect-Mailbox** et **Enable-Mailbox**, qui sont incluses dans ce rôle, sont également regroupées à l’aide des étendues de configuration de la base de données. Ces dernières contrôlent dans quelles bases de données les cmdlets peuvent créer des boîtes aux lettres. La base dans laquelle vous souhaitez créer une boîte aux lettres doit se trouver dans la portée de la base de données. Cette condition s’applique si vous spécifiez une base de données avec le paramètre *Database* dans l’une des deux cmdlet, ou si la distribution automatique des boîtes aux lettres peut sélectionner la base pour vous. Pour plus d’informations, consultez la rubrique [Présentation des portées du rôle de gestion](understanding-management-role-scopes-exchange-2013-help.md).
